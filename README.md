...
```hcl
github.com/pbs/terraform-aws-ecs-service-module?ref=11.2.1
```
...
```hcl
module "service" {
  source = "github.com/pbs/terraform-aws-ecs-service-module?ref=11.2.1"
...
| <a name="module_task"></a> [task](#module\_task) | github.com/pbs-common/terraform-aws-ecs-task-definition-module | 3.0.2 |
...
| <a name="input_cpu_count_scaling"></a> [cpu\_count\_scaling](#input\_cpu\_count\_scaling) | Use CPU CloudWatch metric for scaling | `bool` | `true` | no |
...
| <a name="input_healthcheck_matcher"></a> [healthcheck\_matcher](#input\_healthcheck\_matcher) | The HTTP codes to use when checking for a successful response from a target | `string` | `200` | no |
...
| <a name="input_memory_count_scaling"></a> [memory\_count\_scaling](#input\_memory\_count\_scaling) | Use Memory CloudWatch metric for scaling | `bool` | `false` | no |
...
| <a name="input_virtual_node_cidr_blocks"></a> [virtual\_node\_cidr\_blocks](#input\_virtual\_node\_cidr\_blocks) | CIDR blocks allowed to connect directly to the ECS task (App Mesh virtual node). Empty by default — only set for AppMesh/service mesh use cases. | `list(string)` | `[]` | no |
...
| <a name="output_extra_http_listener_rule_arns"></a> [extra\_http\_listener\_rule\_arns](#output\_extra\_http\_listener\_rule\_arns) | ARNs of the extra HTTP listener rules created |
| <a name="output_extra_https_listener_rule_arns"></a> [extra\_https\_listener\_rule\_arns](#output\_extra\_https\_listener\_rule\_arns) | ARNs of the extra HTTPS listener rules created |
| <a name="output_http_listener_arn"></a> [http\_listener\_arn](#output\_http\_listener\_arn) | ARN of the HTTP listener. Useful when adding extra listener rules to the listener. |
| <a name="output_https_listener_arn"></a> [https\_listener\_arn](#output\_https\_listener\_arn) | ARN of the HTTPS listener. Useful when adding extra ACM certificates or listener rules to the listener. |
